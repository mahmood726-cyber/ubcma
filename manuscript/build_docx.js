...
children.push(captionP("Table 2. ", "Cells dominated (and τ stratification) by AdaptShrink variant on the broadened grids, with mean deployable coverage. Counts use pairwise-complete paired-bootstrap aggregation — each headline-vs-comparator verdict on the reps where both converged — rather than the global all-methods intersection, which dropped whole null cells that low-convergence selection comparators could not run and biased the denominator against null cells. Sources: field2_c2_summary.json, field2_l2_summary.json, field2_*_domination_*.csv, field2_*_scores.csv."));
children.push(tableFrom(
