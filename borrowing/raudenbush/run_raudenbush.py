"""EXPERIMENT 1 -- 5-way real leave-one-trial-out on raudenbush teacher-expectancy (the fair transport test).
...
trial's REAL observed logRR y_t (the truth):
  ...
  3. relevance  : Gaussian weeks-contact-distance kernel x precision, RAW donor logRR
                  (down-weight far-weeks-contact donors; NO standardisation)   [pilot-2 relevance]
  4. transport  : relevance kernel x precision, donor logRR STANDARDISED to the target
                  weeks-contact:  y_s->t = y_s + beta_lat (lat_t - lat_s)      [relevance x transportability]
                  beta_lat = LOO random-effects meta-regression slope from the 12 donors only.
  5. scrambled  : transport but donor weeks-contacts PERMUTED (kernel + shift broken)  [scrambled null]

Because weeks-contact is the ONLY covariate, relevance and transport share the SAME kernel and
...
D = json.load(open(Path(__file__).parent / "raudenbush_trials.json"))
TR = D["trials"]
Y = np.array([t["y"] for t in TR]); S = np.array([t["se"] for t in TR])
LAT = np.array([t["weeks"] for t in TR]); N = len(TR)


def re_slope(lat, y, s):
    """LOO-safe random-effects (DL) meta-regression slope of y on weeks-contact (donors only)."""
    n = len(y)
...
def main():
    xsd = float(LAT.std())
    print(f"raudenbush teacher-expectancy 5-way LOO  (n={N}, weeks-contact SD={xsd:.1f}, RE meta-reg slope on full set="
          f"{re_slope(LAT, Y, S):+.4f}/week)")
    print(f"  TRUTH = real held-out SMD.  Binding: does TRANSPORT beat RELEVANCE-only?\n")
    ...
    json.dump(dict(n=N, x_sd=xsd, rows=rows, verdict=verdict,
                   d_tran_rel_central=dtr), open(Path(__file__).parent / "raudenbush_loo.json", "w"), indent=1)
    print("  wrote raudenbush_loo.json")
