"""EXPERIMENT 1 -- 5-way real leave-one-trial-out on dat.bcg (the fair transport test).
...
trial's REAL observed logRR y_t (the truth):
  ...
  3. relevance  : Gaussian latitude-distance kernel x precision, RAW donor logRR
                  (down-weight far-latitude donors; NO standardisation)   [pilot-2 relevance]
  4. transport  : relevance kernel x precision, donor logRR STANDARDISED to the target
                  latitude:  y_s->t = y_s + beta_lat (lat_t - lat_s)      [relevance x transportability]
                  beta_lat = LOO random-effects meta-regression slope from the 12 donors only.
  5. scrambled  : transport but donor latitudes PERMUTED (kernel + shift broken)  [scrambled null]

Because latitude is the ONLY covariate, relevance and transport share the SAME kernel and
...
D = json.load(open(Path(__file__).parent / "bcg_trials.json"))
TR = D["trials"]
Y = np.array([t["y"] for t in TR]); S = np.array([t["se"] for t in TR])
LAT = np.array([t["ablat"] for t in TR]); N = len(TR)


def re_slope(lat, y, s):
    """LOO-safe random-effects (DL) meta-regression slope of y on latitude (donors only)."""
    n = len(y)
...
def main():
    xsd = float(LAT.std())
    print(f"dat.bcg 5-way LOO  (n={N}, latitude SD={xsd:.1f}, RE meta-reg slope on full set="
    ...
    print(f"  --> {verdict}")
    json.dump(dict(n=N, lat_sd=xsd, rows=rows, verdict=verdict,
                   d_tran_rel_central=dtr), open(Path(__file__).parent / "bcg_loo.json", "w"), indent=1)
    print("  wrote bcg_loo.json")
